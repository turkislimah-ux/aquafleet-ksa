...
- **Balance (prepaid only)**, labelled **"Running Balance"** everywhere it's shown (invoice ledger footers, the customer statement, the KPI strip) — a **continuous derived ledger**: `sum(top-ups) − sum(all consumption)`. Not a stored mutable number. Never resets. Months are only reporting windows over this continuous ledger. **VAT-INCLUSIVE (v3 — reversed from v2):** top-ups are plain money credits with no VAT concept of their own; consumption (a delivered trip, a special charge) draws `amount × 1.15` from the balance — see §5. Balance **can go negative** (a shortfall), shown in the Finance tab in red. Computed by `lib/prepaid.ts`'s `derivedBalanceItems()` — the ONE consumption function; see "Settled Balance" below, which reuses it rather than re-deriving.
...
- **Settled Balance (prepaid only, v3.1, new)** — `sum(top-ups) − sum(consumption of items belonging to a PAID invoice only)`. Unlike Running Balance (which reacts to every delivered trip / added charge, invoiced or not), Settled Balance moves **only on Mark Paid**. It answers "of the balance we've actually cashed out via a paid invoice, what's left" rather than "what's left overall." Shown **only** in the Finance tab's customer-list Balance column (§11) — replacing what used to be the derived/Running figure there. Invoice tables (§9) and the statement keep showing Running Balance; the two are labelled distinctly ("Running Balance" vs "Settled Balance") wherever a reader could otherwise see two different numbers both called "Balance" and assume a bug. Implementation: the exact same `derivedBalanceItems()` call as Running Balance, just fed a pre-filtered slice of trips/charges (only those whose invoice has `status = 'paid'`) — never a second consumption formula.
...
## 9. Invoice structure — v3, updated v3.1 (both modes share the Grand Total shape now)

This section covers the layout/content of the invoice's tables and totals for **both** payment modes — v3.1 unified the Grand Total block and the Amount Due box across prepaid and postpaid (previously postpaid had its own simpler side-by-side layout; see history below).

- **Rows stay pre-VAT** — no per-row VAT column. Each of the Covered and Unpaid tables (both modes) shows its own **Subtotal** line, with a **faded pre-VAT + VAT breakdown** displayed alongside the VAT-inclusive figure (e.g. muted "120.00 + VAT 18.00" next to the bold total) — display-only decomposition of numbers already computed elsewhere, never a re-derivation of the engine. Prepaid's tables additionally carry the **Balance** (relabelled **"Running Balance"**, §2) and **Remaining** lines beneath Subtotal — postpaid has neither (postpaid has no balance concept, §2/§8). On the **Unpaid** table this Remaining figure goes **negative** for prepaid — that's the shortfall that pushed those items to Unpaid in the first place (§5).
- **Special charges** keep their **own table**, styled like the Covered/Unpaid tables and positioned **below** the Unpaid trips table — this table is a distinct block from the **add-a-charge form**, which sits in its own separate, unstyled-as-a-table panel immediately after (v3.1: previously these two were bundled into one bordered panel; now split so the charges table reads as a peer of Covered/Unpaid, and the add-form stays a plain input surface). Order: Covered trips → Unpaid trips → Special charges (table) → Add a charge (form, editable states only).
- **Amount Due box** — sits **before** the Grand Total block (v3.1: previously Amount Due followed Grand Total in prepaid; postpaid had no dedicated Amount Due box at all). Just the figure and the label "Amount Due" — no explanatory sentence (v3.1 dropped "Unpaid trips, informational only — not collectible on this invoice"; the number still means exactly that, it's just not spelled out in the label anymore). The **hide-toggle** (controls whether Amount Due appears in print/PDF/email; always visible here on-screen) now lives at the **top of the Unpaid trips table**, not on the Amount Due box itself (v3.1 relocation, same control/same column). For prepaid, Amount Due = the Unpaid trips subtotal, VAT-inclusive — informational, not collectible on this invoice (unchanged, §5). For postpaid, Amount Due IS the collectible total for the period (postpaid has no covered/rollover split) — numerically identical to Grand Total below it; that's expected, not a bug.
- **Grand Total block** — no title above it anymore (v3.1: dropped "Grand Total" — the block is self-evident). One stacked block, same shape both modes: **Subtotal** (trips) → **Special Charges** → **Total VAT** → **TOTAL**, each on its own line, no Balance/Remaining rows ever (postpaid has no balance; prepaid's balance figures live in the trip tables' own footers, §9 above).
  - **Prepaid**: Subtotal = Covered trips only, pre-VAT. Special Charges = Covered only (an Unpaid special charge rolls forward per §5, doesn't contribute). VAT = 15% on (Subtotal + Special Charges). **Unpaid trips are NOT part of the Grand Total** — they roll to a later invoice (§5), never inflating this invoice's collectible total. (Unchanged from v3.)
  - **Postpaid** (v3.1, new — previously postpaid used a side-by-side Grand-Total/Amount-Due card pair instead of this stacked block): Subtotal = Unpaid trips for the period, pre-VAT. Special Charges = the period's charges, pre-VAT. VAT + TOTAL = the existing document-level `grand` figure (unchanged engine math, §10) — purely a presentation change, no new computation.
...
- A **customer table** listing every customer with: payment mode, period trip count, period billable amount (pre-VAT + VAT), and — v3.1 — the customer's **Settled Balance** (§2) for prepaid customers, labelled distinctly from every other "Balance" surface in the app (postpaid shows "—", no balance concept). The KPI strip above the table still reads **Running Balance** (§2) — the engine number, unaffected by invoice payment status — for its "Total running balance" and over-balance-flag figures (over-balance customers who've exceeded balance, negative margin shown in red, v3 §2/§5). These two coexist deliberately: the KPI strip is an operational/engine view (moves on every delivered trip or added charge), the table column is a settlement view (moves only on Mark Paid).
