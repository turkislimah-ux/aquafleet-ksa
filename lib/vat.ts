...
// lib/money.ts's discipline (pure functions, own test harness before UI).
//
...
// subtotal), never per-item. lib/money.ts's consumption math needs the
// same VAT_RATE constant, so VAT_RATE is now canonically DEFINED in
// lib/money.ts (alongside round2, which already flowed that direction) and
// re-exported from here — one constant, one direction of flow, no circular
// import. This file still never touches lib/money.ts's per-item
...
// NOT banker's rounding (round-half-to-even). Reuses lib/money.ts's round2
...
import { round2, VAT_RATE } from "./money";
