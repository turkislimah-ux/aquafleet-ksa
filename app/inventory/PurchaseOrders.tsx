...
export function ReceiptTypeBadge({ type, lang }: { type: "direct" | "po"; lang: "en" | "ar" }) {
  ...
  return (
    ...
    >
      {isDirect ? (t("inventory.po.direct", lang)) : "PO"}
    </span>
  );
}
...
export function AiPill({ lang }: { lang: "en" | "ar" }) {
  return (
    <span
      ...
      title={t("inventory.po.generatedByAi", lang)}
    >
      {t("inventory.aiPill", lang)}
    </span>
  );
}
  ...
  return (
    ...
    >
      <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wide muted me-1">
        ...
        {t("inventory.po.activeProcurement", lang)}
      </span>
      <ProcChip count={openCount} label={t("inventory.shared.openPos", lang)} onClick={onOpenList} />
      <ProcChip
        count={awaitingReceiptCount}
        label={t("inventory.po.awaitingReceipt", lang)}
        onClick={onOpenReceiveList}
      />
      <ProcChip
        count={pendingReviewCount}
        label={t("inventory.po.pendingReview", lang)}
...
export function suggestAIPurchaseLines(
  ...
  return {
    ...
    lines,
    rationale,
    // Both languages are built here rather than at render because the caller
    // seeds a form field from whichever one matches `lang`. The Arabic branch
    // is chosen by the SAME count as the English one, so the two stay in step.
    noteEn: fill(t(`inventory.po.aiSuggestionNote.${plural(group.length)}`, "en"), { n: group.length }),
    noteAr: fill(t(`inventory.po.aiSuggestionNote.${plural(group.length)}`, "ar"), { n: group.length }),
  };
}
  ...
  function changeWarehouse(newWarehouseId: string) {
    ...
    setLines((prev) => {
      ...
      if (kept.length !== prev.length) {
        setDroppedNotice(
          t("inventory.po.someLinesWere", lang)
        );
      }
      return kept;
    });
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <h2 className="text-lg font-semibold">
                {editingPO
                  ? `${t("inventory.po.editPurchaseOrder", lang)} — ${editingPO.po.po_number}`
                  : t("inventory.shared.newPurchaseOrder", lang)}
              </h2>
              <p className="text-xs muted mt-0.5">
                {t("inventory.po.purchaseOrderInternal", lang)}
              </p>
            </div>
          ...
          {aiSuggestion && (
            ...
            >
              ...
              >
                ...
                {t("inventory.po.generatedByAi", lang)}
              </span>
            ...
            <label className="flex flex-col gap-1 text-sm">
              ...
              {lockWarehouseId && (
                <p className="text-[11px] muted mt-0.5">
                  {t("inventory.po.lockedPartOnly", lang)}
                </p>
              )}
            </label>

            <label className="flex flex-col gap-1 text-sm">
              <span className="muted">{t("inventory.po.expectedDelivery", lang)}</span>
              <input
                type="date"
                ...
                <tbody>
                  {lines.length === 0 ? (
                    <tr>
                      ...
                      >
                        {t("inventory.po.noLineItems", lang)}
                      </td>
                    </tr>
                  ) : (
                    lines.map((l, idx) => {
          ...
          <div className="flex justify-end gap-2">
            ...
            >
              ...
              {saving === "draft"
                ...
                : editingPO
                ? t("inventory.shared.saveChanges", lang)
                : t("inventory.po.saveDraft", lang)}
            </button>
            <button
              type="button"
              disabled={!canSubmit || saving !== null}
            ...
            >
              ...
              {saving === "issued"
                ? t("inventory.po.issuingBusy", lang)
                : editingPO
                ? t("inventory.po.saveIssue", lang)
                : t("inventory.po.issueNow", lang)}
            </button>
          </div>
        </div>
      </div>
    ...
    <ModalOverlay onClick={onClose}>
      ...
      >
        <div className="flex items-start justify-between gap-4 mb-4">
          <h2 className="text-lg font-semibold">{t("inventory.po.openPurchaseOrders", lang)}</h2>
        ...
        {open.length === 0 ? (
          <p className="muted text-sm py-10 text-center">
            {t("inventory.po.noOpenPurchase", lang)}
          </p>
        ) : (
          <Card className="!p-0 overflow-hidden">
            <Table>
              <thead>
                <tr>
                  <TH>{t("inventory.po.poNumber", lang)}</TH>
                  <TH>{t("common.status", lang)}</TH>
                  <TH>{t("inventory.shared.supplier", lang)}</TH>
                  <TH>{t("inventory.po.issuedOn", lang)}</TH>
                  <TH>{t("inventory.po.expectedDelivery", lang)}</TH>
                  <TH>{t("inventory.po.poTotalIncl", lang)}</TH>
                  <TH></TH>
                </tr>
              </thead>
              <tbody>
      ...
      >
        ...
        {po.ai_generated && (po.ai_rationale || po.ai_rationale_ar) && (
          ...
          >
            ...
            {/* The guard above admits this block when EITHER column is set, so
                picking one by language alone renders an empty bubble under a
                "Generated by AI" badge whenever the other is the one that
                exists. That failed in BOTH directions, not just Arabic.
                `arText` is the app-wide answer: preferred language when it is
                there, the other column when it is not, never blank. The `?? ""`
                is unreachable under the guard and exists only to satisfy
                `arText`'s non-null `base`. */}
            <span className="text-xs leading-snug">
              {arText(po.ai_rationale ?? po.ai_rationale_ar ?? "", po.ai_rationale_ar, lang)}
            </span>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm mb-4">
          <div>
            <div className="text-[11px] muted uppercase">{t("inventory.po.poNumber", lang)}</div>
            <div className="font-mono font-medium">{po.po_number}</div>
          </div>
          <div>
            <div className="text-[11px] muted uppercase">{t("common.status", lang)}</div>
            <div className="mt-0.5">
              <PoStatusPill status={po.status} lang={lang} />
            </div>
          </div>
          <div>
            <div className="text-[11px] muted uppercase">{t("inventory.po.issuedOn", lang)}</div>
            <div className="font-medium">{po.request_date}</div>
          </div>
          <div>
            <div className="text-[11px] muted uppercase">{t("inventory.po.expectedDelivery", lang)}</div>
            <div className="font-medium">{po.expected_delivery ?? "—"}</div>
          </div>
          <div>
            <div className="text-[11px] muted uppercase">{t("inventory.po.requestedBy", lang)}</div>
            <div className="font-medium">{po.requested_by ?? "—"}</div>
          </div>
          <div>
            <div className="text-[11px] muted uppercase">{t("inventory.po.receivedBy", lang)}</div>
            <div className="font-medium">{po.received_by ?? "—"}</div>
          </div>
          <div>
            <div className="text-[11px] muted uppercase">{t("inventory.shared.receivedOn", lang)}</div>
                  ...
                  <TD className="tabular">
                    ...
                    {l.received_qty != null && l.received_qty !== l.qty && (
                      <span className="muted text-[11px] ms-1">
                        ({t("inventory.po.orderedSuffix", lang)}: {l.qty})
                      </span>
                    )}
                  </TD>
                  <TD className="tabular">
                    ...
                    {l.received_unit_price_sar != null && l.received_unit_price_sar !== l.unit_price_sar && (
                      <span className="muted text-[11px] ms-1">
                        ({t("inventory.po.orderedSuffix", lang)}: {formatSar(l.unit_price_sar)})
                      </span>
                    )}
                  </TD>
        ...
        {po.note && (
          <Card className="!p-3 mb-4">
            <div className="text-[11px] muted uppercase mb-1">{t("common.note", lang)}</div>
        ...
        {showApprovals && (
          <Card className="!p-3 mb-4">
            <h4 className="font-semibold text-sm mb-2 flex items-center gap-2">
              ...
              {t("inventory.po.approvedBy", lang)}{" "}
              <span className="muted text-xs font-normal">({approvals.length}/2)</span>
            </h4>
            {approvals.length === 0 ? (
              <p className="muted text-sm">{t("inventory.po.awaitingApproval", lang)}</p>
            ) : (
            ...
            {po.status === "rejected" && (
              <div className="mt-3 pt-3 border-t" style={{ borderColor: "rgb(var(--border))" }}>
                <div className="font-semibold text-sm text-rose-600">
                  {t("inventory.po.rejectedBy", lang)}: {po.rejected_by ?? "—"}
                </div>
          ...
          {po.status === "draft" && (
            <>
              ...
              >
                ...
                {issuing ? (t("inventory.po.issuingBusy", lang)) : t("inventory.po.issue", lang)}
              </button>
            </>
          )}
          {po.status === "issued" && (
            ...
            >
              ...
              {t("inventory.po.receiveStock", lang)}
            </button>
          )}
  ...
  return (
    <ModalOverlay onClick={onClose}>
      ...
      >
        <div className="flex items-start justify-between gap-4 mb-4">
          <h2 className="text-lg font-semibold">
            {t("inventory.po.purchaseOrdersAwaiting", lang)}
          </h2>
          <button type="button" onClick={onClose} className="p-1 rounded hover:bg-black/5 dark:hover:bg-white/5">
            <X className="h-4 w-4" />
          </button>
        ...
        {issued.length === 0 ? (
          <p className="muted text-sm py-10 text-center">
            {t("inventory.po.noPosAwaiting", lang)}
          </p>
        ) : (
          <>
            <p className="text-sm muted mb-3">
              {t("inventory.po.pickIssuedPo", lang)}
            </p>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-[11px] muted mb-2">
                      {t("inventory.po.expectedDelivery", lang)}: {po.expected_delivery ?? "—"}
                    </div>
                  </button>
                );
              })}
...
export function ReceivePOModal({
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!linesValid || receiveLines.length === 0) {
      setError(
        t("inventory.po.everyLineNeeds", lang)
      );
      return;
    }
    if (files.length === 0) {
      ...
      >
        <form onSubmit={submit} className="flex flex-col gap-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h2 className="text-lg font-semibold">
                {t("inventory.po.receivePurchaseOrder", lang)} — {po.po_number}
              </h2>
              <p className="text-xs muted mt-0.5">
                {t("inventory.po.confirmWhatActually", lang)}
              </p>
            </div>
            <button type="button" onClick={close} className="p-1 rounded hover:bg-black/5 dark:hover:bg-white/5">
              <X className="h-4 w-4" />
          ...
          >
            ...
            <div className="text-sm">
              ...
              >
                ...
                {t("inventory.po.detachPurchaseOrder", lang)}
              </button>
              <p className="text-xs muted mt-0.5">
                {detached
                  ? fill(t("inventory.po.looseReceiptDetached", lang), { po: po.po_number })
                  : t("inventory.po.leaveUncheckedReceive", lang)}
              </p>
            </div>
          </div>
          ...
          <Card className="!p-0 overflow-hidden">
            <Table>
              <thead>
                <tr>
                  <TH>{t("common.part", lang)}</TH>
                  <TH>{t("inventory.po.orderedQty", lang)}</TH>
                  <TH>{t("inventory.po.orderedUnitPrice", lang)}</TH>
                    ...
                    <tr key={l.key}>
                      <TD>
                        ...
                        {l.line_id === null && (
                          ...
                          >
                            {t("inventory.po.extraNotPo", lang)}
                          </span>
                        )}
                      </TD>
                        ...
                        <div className="inline-flex items-center gap-1.5">
                          {l.line_id !== null && (
                            ...
                            >
                              ...
                              />
                              {variance
                                ? t("inventory.po.thVariance", lang)
                                : t("inventory.po.match", lang)}
                            </span>
                          )}
                          {canRemoveLine(l) && (
                            <button
              ...
              <PartPicker
                ...
                placeholder={
                  availableExtraParts.length === 0
                    ? t("inventory.po.noOtherParts", lang)
                    : t("inventory.shared.pickPartAdd", lang)
                }
              />
            </div>
            <Btn type="button" variant="outline" onClick={addExtraLine} disabled={!addPartId}>
              <Plus className="h-4 w-4" />
              {t("inventory.po.addExtraPart", lang)}
            </Btn>
          </div>
          ...
          <div className="flex justify-end gap-2">
            ...
            >
              ...
              {saving
                ? t("common.saving", lang)
                : detached
                ? t("inventory.po.saveLooseReceipt", lang)
                : t("inventory.po.confirmReceipt", lang)}
            </button>
          </div>
        </form>
      </div>
...
function approvalRowTitle(row: ApprovalRow, lang: "en" | "ar"): string {
  ...
  return `${t("inventory.po.directReceipt", lang)} — ${row.receipt.id.slice(0, 8).toUpperCase()}`;
}
...
function VoteSummary({ approvals, lang }: { approvals: StockReceiptApproval[]; lang: "en" | "ar" }) {
  if (approvals.length === 0) {
    return <span className="muted text-xs">{t("inventory.po.noVotesYet", lang)}</span>;
  }
  return (
    <div className="flex flex-col gap-0.5">
      {approvals.map((a) => (
        <div key={a.id} className="text-[11px]">
          ...
          >
            {a.action === "approve" ? (t("inventory.po.approve", lang)) : (t("inventory.po.reject", lang))}
          </span>
          {a.outcome && (
      ...
      {approvals.length === 1 && (
        <span className="text-[10px] muted italic">
          {t("inventory.po.awaitingMatchingSecond", lang)}
        </span>
      )}
    </div>
  );
...
export function ApproveReceiptModal({
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault();
    if (!receiptId) {
      setError(t("inventory.po.noReceiptLinked", lang));
      return;
    }
    setSaving(true);
    setError(null);
    ...
    if (live.confirmedNotPending) {
      setSaving(false);
      setError(
        t("inventory.po.hasAlreadyBeen", lang)
      );
      return;
    }
        ...
        <form onSubmit={submit}>
          <div className="flex items-start justify-between gap-4 mb-1">
            <h2 className="text-lg font-semibold inline-flex items-center gap-2 flex-wrap">
              {t("inventory.po.approve", lang)} — {approvalRowTitle(row, lang)}
          ...
          <p className="text-sm muted mb-2">
            {t("inventory.po.bothApproversMust", lang)}
          </p>
          <div className="mb-3">
            ...
            <p className="text-xs muted mb-3 italic">
              {t(
                myVote.action === "approve"
                  ? "inventory.po.voteAlreadyApproved"
                  : "inventory.po.voteRejectBecomesApprove",
                lang
              )}
            </p>
          )}
          ...
          <label className="flex flex-col gap-1 text-sm">
            <span className="muted">{t("inventory.po.optionalComment", lang)}</span>
            <input
              value={comment}
              onChange={(e) => setComment(e.target.value)}
              className={INPUT}
              style={INPUT_STYLE}
              placeholder={t("inventory.po.optionalComment", lang)}
            />
          </label>
          ...
          <div className="mt-5 flex justify-end gap-2">
            ...
            >
              <Check className="h-4 w-4" />
              {saving ? (t("common.saving", lang)) : t("inventory.po.approve", lang)}
            </button>
          </div>
        </form>
      </div>
    ...
    <ModalOverlay onClick={close}>
      ...
      >
        <form onSubmit={submit}>
          <div className="flex items-start justify-between gap-4 mb-1">
            <h2 className="text-lg font-semibold inline-flex items-center gap-2 flex-wrap">
              {t("inventory.po.reject", lang)} — {approvalRowTitle(row, lang)}
              <ReceiptTypeBadge type={row.kind === "po" ? "po" : "direct"} lang={lang} />
            </h2>
            <button type="button" onClick={close} className="p-1 rounded hover:bg-black/5 dark:hover:bg-white/5">
              <X className="h-4 w-4" />
          ...
          <p className="text-sm muted mb-2">
            {t("inventory.po.bothApproversMustPick", lang)}
          </p>
          <div className="mb-3">
            <VoteSummary approvals={approvals} lang={lang} />
          </div>
          ...
          {myVote && (
            <p className="text-xs muted mb-3 italic">
              {myVote.action === "reject"
                ? fill(t("inventory.po.voteAlreadyRejected", lang), {
                    outcome: myVote.outcome ? OUTCOME_LABEL[myVote.outcome][lang] : "",
                  })
                : t("inventory.po.voteApproveBecomesReject", lang)}
            </p>
          )}

          <div className="flex flex-col gap-2 text-sm mb-4">
            <span className="muted">{t("inventory.po.rejectionOutcome", lang)}</span>
            ...
            >
              ...
              <span>
                <span className="font-medium block">
                  {t("inventory.po.keepPartsVoid", lang)}
                </span>
                <span className="text-xs muted">
                  {t("inventory.po.stockStaysHand", lang)}
                </span>
              </span>
            </label>
            ...
            >
              ...
              <span>
                <span className="font-medium block">
                  {t("inventory.po.removeStockEntirely", lang)}
                </span>
                <span className="text-xs muted">
                  {t("inventory.po.reversesStockReceipt", lang)}
                </span>
              </span>
            </label>
          </div>
          ...
          <label className="flex flex-col gap-1 text-sm">
            <span className="muted">{t("inventory.po.rejectionReason", lang)}</span>
            <textarea
              value={reason}
          ...
          <div className="mt-5 flex justify-end gap-2">
            ...
            >
              <X className="h-4 w-4" />
              {saving ? (t("common.saving", lang)) : t("inventory.po.reject", lang)}
            </button>
          </div>
        </form>
      </div>
...
export function ApprovalsTab({
  ...
  async function stillPending(row: ApprovalRow): Promise<boolean> {
    ...
    if (live.confirmedNotPending) {
      setNotice(
        t("inventory.po.wasAlreadyResolved", lang)
      );
      router.refresh();
      return false;
    }
  ...
  return (
    <Card className="!p-0 overflow-hidden">
      <div className="p-3 border-b" style={{ borderColor: "rgb(var(--border))" }}>
        <h3 className="font-semibold">{t("inventory.po.approvalsQueue", lang)}</h3>
        <p className="text-sm muted">
          {t("inventory.po.bothApproversMustCast", lang)}
        </p>
        {notice && <p className="text-xs text-amber-600 dark:text-amber-400 mt-1">{notice}</p>}
      </div>
      {queue.length === 0 ? (
        <p className="muted text-sm py-6 text-center">
          {t("inventory.po.nothingAwaitingApproval", lang)}
        </p>
      ) : (
        <Table>
          <thead>
            <tr>
              <TH>{t("inventory.po.reference", lang)}</TH>
              <TH>{t("inventory.shared.supplier", lang)}</TH>
              <TH>{t("inventory.shared.receivedOn", lang)}</TH>
              <TH>{t("inventory.po.actualTotalIncl", lang)}</TH>
              <TH>{t("inventory.po.votes", lang)}</TH>
              <TH></TH>
            </tr>
          </thead>
          <tbody>
...
export function ReceiptDetailModal({
  ...
  async function stillPending(): Promise<boolean> {
    ...
    if (live.confirmedNotPending) {
      setNotice(
        t("inventory.po.hasAlreadyBeenResolved", lang)
      );
      router.refresh();
      return false;
    }
  ...
  return (
    <ModalOverlay onClick={onClose}>
      ...
      >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm mb-4">
          <div>
            <div className="text-[11px] muted uppercase">{t("inventory.po.reference", lang)}</div>
                    ...
                    <TD>
                      ...
                      {l.price_lot_id === null && (
                        <span
                          ...
                          title={
                            t("inventory.po.noTraceablePrice", lang)
                          }
                        >
                          {t("inventory.po.notTraceable", lang)}
                        </span>
                      )}
                    </TD>
        ...
        <Card className="!p-3 mb-4">
          ...
          {receipt.status === "rejected" && (
            <div className="mt-3 pt-3 border-t" style={{ borderColor: "rgb(var(--border))" }}>
              <div className="font-semibold text-sm text-rose-600">
                {t("inventory.po.rejectedBy", lang)}: {receipt.rejected_by ?? "—"}
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Stat label={t("inventory.po.spend30d", lang)} value={formatSar(spend30)} tone="info" />
        <Stat label={t("inventory.po.spend90d", lang)} value={formatSar(spend90)} tone="info" />
        <Stat label={t("inventory.shared.inventoryValue", lang)} value={formatSar(inventoryValue)} tone="ok" />
        <Stat label={t("inventory.shared.openPos", lang)} value={openPOsCount} tone="warn" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card className="!p-4">
          <h3 className="font-semibold mb-3">{t("inventory.po.topSpendCategories", lang)}</h3>
          {catRows.length === 0 ? (
            <p className="muted text-sm">{t("inventory.po.noApprovedSpend", lang)}</p>
          ) : (
        ...
        <Card className="!p-4">
          <h3 className="font-semibold mb-3">{t("inventory.po.spendBySupplier", lang)}</h3>
          {supRows.length === 0 ? (
            <p className="muted text-sm">{t("inventory.po.noApprovedSpend", lang)}</p>
          ) : (
            <div className="space-y-2">
      ...
      >
        <div className="flex items-center gap-2 mb-3">
          ...
          >
            <Zap className="h-3 w-3" />
            {t("inventory.po.aiInsightsRecommendations", lang)}
          </span>
        </div>
        <div className="space-y-3">
          {lowParts.length > 0 && (
            <div className="rounded-lg border p-3" style={{ background: "rgba(245,158,11,.06)", borderColor: "rgba(245,158,11,.3)" }}>
              <div className="font-medium text-sm flex items-center">
                {t("inventory.po.itemsBelowReorder", lang)}
          ...
          {pricedUp.length > 0 && (
            <div className="rounded-lg border p-3" style={{ background: "rgba(11,126,234,.05)", borderColor: "rgba(11,126,234,.25)" }}>
              <div className="font-medium text-sm flex items-center">
                {t("inventory.po.partsWhoseLatest", lang)}
                <span className="inline-block min-w-[1.25rem] px-1.5 rounded-full bg-violet-500 text-white text-[11px] font-bold text-center ms-2">
                  {pricedUp.length}
                </span>
              </div>
          ...
          {consolidate.length > 0 && (
            <div className="rounded-lg border p-3" style={{ background: "rgba(11,126,234,.05)", borderColor: "rgba(11,126,234,.25)" }}>
              <div className="font-medium text-sm">
                {t("inventory.po.suppliersMultipleSmall", lang)}
              </div>
          ...
          {noInsights && (
            <p className="muted text-sm">
              {t("inventory.po.noRecommendationsTime", lang)}
            </p>
          )}
        </div>
      </div>
    ...
    <div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm mb-3">
        <div className="rounded-lg border p-3" style={INPUT_STYLE}>
          <div className="text-[11px] muted uppercase">{t("inventory.po.purchases", lang)}</div>
        ...
        <div className="rounded-lg border p-3" style={INPUT_STYLE}>
          <div className="text-[11px] muted uppercase">
            {t("inventory.po.consumption", lang)} ({t("inventory.po.allTime", lang)})
          </div>
        ...
        <div className="rounded-lg border p-3" style={INPUT_STYLE}>
          <div className="text-[11px] muted uppercase">{t("inventory.shared.stockValue", lang)}</div>
          <div className="text-lg font-semibold tabular-nums text-brand-600">{formatSar(stockValue)}</div>
          <div className="text-[11px] muted">
            {part.qty_on_hand} {part.unit ?? ""} {t("inventory.po.inStock", lang)}
          </div>
        </div>
        <div className="rounded-lg border p-3" style={INPUT_STYLE}>
          <div className="text-[11px] muted uppercase">{t("inventory.po.priceTrend", lang)}</div>
          <div className={cn("text-lg font-semibold tabular-nums", trendCls)}>
            {trendArrow} {Math.abs(priceTrendPct)}%
          </div>
          <div className="text-[11px] muted">{t("inventory.po.vsHistoricalBatches", lang)}</div>
        </div>
      </div>
    ...
    <ModalOverlay onClick={onClose}>
      ...
      >
        ...
        <div>
          <h4 className="font-semibold text-sm mb-2 flex items-center gap-2">
            <ShoppingCart className="h-4 w-4" />
            {t("inventory.po.purchaseHistory", lang)}
          </h4>
          <Card className="!p-0 overflow-hidden">
            <Table>
              <thead>
                <tr>
                  ...
                  <TH>{t("inventory.po.thPoNumber", lang)}</TH>
                  <TH>{t("common.qty", lang)}</TH>
                  <TH>{t("inventory.shared.unitCost", lang)}</TH>
                ...
                {purchaseRows.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="py-6 px-3 border-t text-center muted text-sm" style={{ borderColor: "rgb(var(--border))" }}>
                      {t("inventory.po.noPurchasesYet", lang)}
                    </td>
                  </tr>
                ) : (
