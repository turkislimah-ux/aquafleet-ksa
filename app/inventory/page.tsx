...
import type { Warehouse, Part, PriceLot, Supplier, Unit, PurchaseOrder, PurchaseOrderLine } from "@/lib/db-types";
...
export default async function InventoryPage() {
  ...
  const [warehousesRes, partsRes, priceLotsRes, suppliersRes, unitsRes, purchaseOrdersRes, purchaseOrderLinesRes] =
    await Promise.all([
      supabase
        .from("warehouses")
      ...
      // Full-demo Phase 4 (migration 0050) — Purchase Orders core. Bulk read
      // (display only — same "fetch everything, derive client-side" pattern
      // price_lots above already uses) so the proc-strip chip / PO list /
      // detail can all compute counts and totals without an extra
      // round-trip per PO. No stored total column exists to select — every
      // total is derived from purchaseOrderLines at render.
      supabase
        .from("purchase_orders")
        .select(
          "id, po_number, supplier_id, warehouse_id, status, request_date, expected_delivery, note, requested_by, issued_at, created_at"
        )
        .order("created_at", { ascending: false }),
      supabase
        .from("purchase_order_lines")
        .select("id, purchase_order_id, part_id, qty, unit_price_sar, created_at"),
    ]);
