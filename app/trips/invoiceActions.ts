...
// ONE PAYLOAD, TWO DOCUMENTS.
//
// This was the body of getInvoicePdf until the PRINT sheet became its own
// template (lib/invoicePrintTemplate.ts). It is extracted rather than copied
// for the same reason lib/invoiceViewModel.ts exists: the draft-vs-frozen
// branch below is where an invoice decides whether it recomputes live or reads
// its 0027 freeze, and a second copy of that decision is a second answer to
// "what does this invoice say" — free to drift, and only visible to whoever
// happens to compare a printout against a downloaded PDF.
//
...
async function toPdfInvoiceData(
  ...
  invoiceId: string,
  inv: Invoice,
): Promise<ActionResult<PdfInvoiceData>> {
  ...
  // anywhere. It sits here, before either branch, and both callers invoke this
  // function only after their own short-circuits — so a PDF cache hit still
  // costs zero project reads.
  //
  ...
  return { error: null, data: pdfData };
}
...
export async function getInvoicePdf(invoiceId: string): Promise<ActionResult<InvoicePdfResult>> {
  ...
  const { error: dataErr, data: pdfData } = await toPdfInvoiceData(supabase, invoiceId, inv);
  if (dataErr || !pdfData) return { error: dataErr ?? "Could not assemble invoice for PDF." };
...
// ---------------------------------------------------------------------------
// Print — the plain sheet, rendered here and printed by the user's own browser
// ---------------------------------------------------------------------------
// Returns HTML, not a PDF: printing is the BROWSER's job and it does it for
// free, so there is no provider to call, nothing to pay for, and no failure
// mode where Ctrl+P depends on a third-party service being up. The client
// writes this into a hidden same-origin iframe and prints that.
//
// It renders SERVER-SIDE because that is where the document already lives —
// the freeze branch, the assembly and the QR encoder are all here. The
// alternative (ship the raw invoice to the client and render it there) would
// put a second assembly in the browser, which is the divergence this whole
// chain is built to prevent.
//
// NO CACHE, deliberately, and it needs none. The download caches because each
// generation costs a provider call; this costs a string. More to the point, a
// cache is what made the hide-amount-due toggle stale for downloads (see
// setHideAmountDue) — nothing here can go stale, because nothing is stored.
export async function getInvoicePrintHtml(invoiceId: string): Promise<ActionResult<{ html: string }>> {
  const supabase = createClient();
  ...
  const { error: dataErr, data: pdfData } = await toPdfInvoiceData(supabase, invoiceId, inv);
  if (dataErr || !pdfData) return { error: dataErr ?? "Could not assemble invoice for printing." };

  return { error: null, data: { html: await buildInvoicePrintHtml(pdfData) } };
}
