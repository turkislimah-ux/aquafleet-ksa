...
| **Stage B (this session)** | Receipt approval + Direct invoices, two-vote model, lot-link fix, UI fixes, cleanup, null-`price_lot_id` root-cause fix | `16e3f02` (migrations `0057`/`0058`), `659c4bc` (app code), `256d10d` (cleanup), `54531a2` (migration `0059`) |

---

## 6. Open items / known limitations / assumptions

- **Untraceable-lines guard — RESOLVED by migration `0059`, applied and verified.** The null-`price_lot_id` growth flagged earlier this session was root-caused: `receive_loose_parts` was finding its lot via a post-hoc `select ... order by created_at desc limit 1` guess instead of a direct reference, and it was provably wrong on live data (5 rows had a demonstrably matching, unclaimed lot yet were never linked to it). Fixed by changing `add_price_lot` to `RETURNS uuid` (the exact lot it just inserted) and having `receive_loose_parts` capture that value directly — no lookup at all now, on either the Direct or PO path (PO composes on `receive_loose_parts`, so one fix covers both). The 5 real-bug rows were backfilled via an unambiguous match (part_id + qty_purchased + price_sar + created_at, unclaimed lot only); a separate, genuinely-correct bucket of 4 rows (`stock_receipt_lines_price_lot_id_fkey` is `ON DELETE SET NULL`, and `reject_stock_receipt`'s `remove_stock` branch deletes those exact lots) stay null on purpose — not a defect, just the correct terminal state for a receipt whose stock was reversed. Turki's post-migration smoke test confirmed fresh receives link correctly on both paths; FIFO invariant re-verified at 0. **Only the 4 remove_stock-rejected rows are null now, and that's permanent/correct** — nothing further to do here.
