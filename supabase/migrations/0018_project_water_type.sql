-- 0018_project_water_type.sql
-- Thread water_type through BOTH project RPCs (create 0016 / update 0017).
-- WHY: projects.water_type has always existed (0003, nullable, CHECK potable|
-- non_potable) but neither RPC wrote it, so every project landed with water_type
-- = NULL and the trip form's project→trip inheritance was inert. This adds a
-- single p_water_type text param to each function and writes it; NOTHING ELSE
-- changes (derive-customer, column-by-column writes, driver diff, SECURITY
-- INVOKER, trips-never-touched all preserved verbatim).
--
-- OVERLOAD HANDLING: adding a parameter changes the function SIGNATURE, so
-- `create or replace` would leave a SECOND (stale) overload behind. We DROP the
-- old signatures first (by their exact OLD arg-type lists), then create the new
-- ones, then re-issue grant execute for the NEW arg-type lists.
--
-- Column stays NULLABLE — existing rows with water_type = NULL are untouched;
-- the field is made required at the app layer only.

begin;

-- 1) Drop the OLD signatures (pre-water_type). --------------------------------
...
-- 2) Recreate create_project_with_customer WITH p_water_type. -----------------
...
begin
  ...
  --    intentionally left NULL. water_type is now written from p_water_type.
...
-- 3) Recreate update_project_with_customer WITH p_water_type. -----------------
...
begin
  ...
  -- 2) Project — ONLY the form-captured columns (now incl. water_type).
